...
function buildPreviewGroups(style: CaptionGroupStyle): CaptionGroupProp[] {
  let cursor = 0;
  return PREVIEW_PHRASES.map((texts) => {
    const startFrame = cursor;
    const words: CaptionWordProp[] = texts.map((text, i) => {
    ...
    return { words, startFrame, endFrame, captionsAtATime: style.captionsAtATime, style };
  });
}
