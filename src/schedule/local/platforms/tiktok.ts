...
import {
  ...
  settle,
  withBrowser,
} from "~/schedule/local/browser";
import { TIKTOK } from "~/schedule/local/selectors";
...
function parseMonthHeader(text: string): { year: number; month: number } | null {
  ...
  const match =
    /\b(January|February|March|April|May|June|July|August|September|October|November|December)\s*\/\s*(\d{4})\b/i.exec(
      text.replace(/\u00a0/g, " "),
    );
...
export function createTikTokPublisher(timeZone: string): Publisher {
  return {
    id: "tiktok",
