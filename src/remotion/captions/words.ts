...
/**
 * Group words for captions within sentence boundaries.
...
export function groupStyledCaptionWords(
  ...
  const runBoundary = (word: (typeof words)[number]): string =>
  ...
  const flushRun = () => {
    ...
    const plain: CaptionWordProp[] = run.map((w) => ({
      text: w.text,
